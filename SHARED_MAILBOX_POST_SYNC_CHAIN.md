...
- 当前 shared-mailbox 入口在 inspection 开始前，仍然要求单独提供 `EMPLOYEE_INFO_URL`
- `inspect_task_upload_assignments()` 现在也是从这张员工信息表读取负责人目录，不是只靠 task-upload record 自带字段就能启动
- 所以如果只给 task-upload，不给员工信息表，这条入口会先报 `缺少 EMPLOYEE_INFO_URL`
...
另外，shared-mailbox 这条入口当前只把 downstream 的下面两种状态当成成功：

- `completed`
- `completed_with_partial_scrape`

不要把 `staged_only` 当成这条入口里已经验证可 live writeback 的成功态。
像 `--skip-scrape` 这类只停在 staging 的结果，在当前实现里仍会让任务落到 skipped/failed 处理，不应该按“这轮已经安全跑完”理解。
...
- `AI回信管理` 子表
  目标表必须存在这个名字的子表。
- 员工信息里的英文名 / 邮箱
  当前负责人路由要靠这个识别 `Lilith / Rhea` 这类邮件归属；运行时仍通过单独的 `EMPLOYEE_INFO_URL` 读取这张员工信息表。
