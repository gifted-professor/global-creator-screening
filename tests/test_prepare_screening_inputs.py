...
class PrepareScreeningInputsTests(unittest.TestCase):
    ...
    def test_prepare_screening_inputs_accepts_mail_thread_funnel_workbook(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(
                tiktok_metadata["livio.official"]["resolution_evidence"],
                "greeting:livio.official",
            )
            self.assertEqual(
                tiktok_metadata["livio.official"]["candidate_sources"],
                '["greeting_quoted"]',
            )
            self.assertEqual(
                tiktok_metadata["high.confidence.creator"]["business_signal_detected"],
                "true",
            )
            self.assertNotIn("medium.confidence.creator", tiktok_metadata, tiktok_metadata)
            self.assertEqual(
